...
and d_date between '1998-03-12' and 
        (cast('1998-03-12' as date) + 90 days)
...
and d_date between '1998-01-18' and 
        (cast('1998-01-18' as date) + 90 days)
...
select  i_item_id
 ...
 and d_date between cast('2002-05-22' as date) 
 				and (cast('2002-05-22' as date) + 30 days)
 group by i_item_id
         ,i_item_desc 
         ,i_category
         ,i_class
...
select  *
 from(select w_warehouse_name
            ,i_item_id
            ,sum(case when (cast(d_date as date) < cast ('2000-03-10' as date))
   ...
   where i_current_price between 0.99 and 1.49
     ...
     and inv_warehouse_sk   = w_warehouse_sk
     and inv_date_sk    = d_date_sk
     and d_date between (cast ('2000-03-10' as date) - 30 days)
                    and (cast ('2000-03-10' as date) + 30 days)
...
select  i_item_id
 ...
 and d_date between cast('2002-03-05' as date) and (cast('2002-03-05' as date) +  60 days)
...
select  
   w_state
  ,i_item_id
  ,sum(case when (cast(d_date as date) < cast ('1998-02-06' as date)) 
 ...
 and cs_warehouse_sk    = w_warehouse_sk 
 and cs_sold_date_sk    = d_date_sk
 and d_date between (cast ('1998-02-06' as date) - 30 days)
                and (cast ('1998-02-06' as date) + 30 days) 
 group by
    w_state,i_item_id
...
select  i_item_id
 ...
 and inv_item_sk = i_item_sk
 and d_date_sk=inv_date_sk
 and d_date between cast('1998-02-10' as date) and (cast('1998-02-10' as date) +  60 days)
...
with sr_items as
 ...
 and   d_date    in 
	...
	where d_week_seq in 
		(select d_week_seq
		from date_dim
	  where d_date in ('2000-07-09','2000-08-03','2000-11-18')))
...
where 
	ws_item_sk = i_item_sk 
  	...
  	and ws_sold_date_sk = d_date_sk
	and d_date between cast('2000-06-13' as date) 
...
with ss as
 ...
 where ss_sold_date_sk = d_date_sk
       and d_date between cast('1999-08-14' as date) 
                  and (cast('1999-08-14' as date) +  30 days) 
       and ss_store_sk = s_store_sk
 group by s_store_sk)
 ,
 sr as
