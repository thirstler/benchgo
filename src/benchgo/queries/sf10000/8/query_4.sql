...
and d_date between '2001-01-15' and 
        (cast('2001-01-15' as date) + 90 days)
...
with sr_items as
 ...
 and   d_date    in 
	...
	where d_week_seq in 
		(select d_week_seq
		from date_dim
	  where d_date in ('1998-06-01','1998-09-14','1998-11-07')))
...
select  *
 from(select w_warehouse_name
   ...
   where i_current_price between 0.99 and 1.49
     ...
     and inv_warehouse_sk   = w_warehouse_sk
     and inv_date_sk    = d_date_sk
     and d_date between (cast ('1998-04-07' as date) - 30 days)
                    and (cast ('1998-04-07' as date) + 30 days)
...
select  
   w_state
  ,i_item_id
  ,sum(case when (cast(d_date as date) < cast ('1998-04-14' as date)) 
 ...
 and cs_sold_date_sk    = d_date_sk
 and d_date between (cast ('1998-04-14' as date) - 30 days)
                and (cast ('1998-04-14' as date) + 30 days) 
 group by
    w_state,i_item_id
...
select  i_item_id
 ...
 and d_date between cast('1998-01-21' as date) and (cast('1998-01-21' as date) +  60 days)
...
and d_date between '1999-02-23' and 
        (cast('1999-02-23' as date) + 90 days)
...
where 
	ws_item_sk = i_item_sk 
  	...
  	and ws_sold_date_sk = d_date_sk
	and d_date between cast('1998-05-28' as date) 
				and (cast('1998-05-28' as date) + 30 days)
group by 
	i_item_id
        ,i_item_desc 
        ,i_category
...
select  i_item_id
 ...
 and d_date between cast('2001-01-07' as date) 
 				and (cast('2001-01-07' as date) + 30 days)
 group by i_item_id
         ,i_item_desc 
         ,i_category
         ,i_class
...
where 
	...
	and d_date between cast('1999-05-17' as date) 
				and (cast('1999-05-17' as date) + 30 days)
group by 
	i_item_id
        ,i_item_desc 
        ,i_category
...
with ssr as
 ...
 where date_sk = d_date_sk
       and d_date between cast('1998-08-28' as date) 
                  and (cast('1998-08-28' as date) +  14 days)
