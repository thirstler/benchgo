...
# Spark queries in order
SPARK_TPCDS_QUERY_SF1000={"query1": """
...
where 
	ws_item_sk = i_item_sk 
  	...
  	and ws_sold_date_sk = d_date_sk
	and d_date between cast(date('2001-01-12') as date) 
				and (cast(date('2001-01-12') as date) + interval '30' day)
group by 
	i_item_id
        ,i_item_desc 
        ,i_category
...
and d_date between date(date('1998-03-18')) and 
        (cast(date('1998-03-18') as date) + interval '90' day)
...
select  i_item_id
 ...
 and d_date between cast(date('2001-06-02') as date) and (cast(date('2001-06-02') as date) + interval '60' day)
...
select  i_item_id
 ...
 and inv_item_sk = i_item_sk
 and d_date_sk=inv_date_sk
 and d_date between cast(date('2002-05-30') as date) and (cast(date('2002-05-30') as date) + interval '60' day)
...
with sr_items as
 ...
 and   d_date    in 
	...
	where d_week_seq in 
		(select d_week_seq
		from date_dim
	  where d_date in (date('1998-01-02'),date('1998-10-15'),date('1998-11-10'))))
...
SPARK_TPCDS_QUERY_SF10000={"query1": """
with customer_total_return as
(select sr_customer_sk as ctr_customer_sk
,sr_store_sk as ctr_store_sk
,sum(SR_FEE) as ctr_total_return
