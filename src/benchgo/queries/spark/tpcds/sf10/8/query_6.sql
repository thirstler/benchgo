...
with ssr as
 ...
 where date_sk = d_date_sk
       and d_date between cast(date('2000-08-21') as date) 
                  and (cast(date('2000-08-21') as date) + interval '14' day)
...
select  i_item_id
 ...
 where cs_item_sk = i_item_sk 
   ...
   and cs_sold_date_sk = d_date_sk
 and d_date between cast(date('2000-04-19') as date) 
 				and (cast(date('2000-04-19') as date) + interval '30' day)
 group by i_item_id
         ,i_item_desc 
         ,i_category
         ,i_class
...
and d_date between date(date('1998-01-19')) and 
        (cast(date('1998-01-19') as date) + interval '90' day)
...
select  i_item_id
 ...
 and d_date between cast(date('2002-03-12') as date) and (cast(date('2002-03-12') as date) + interval '60' day)
...
where 
	ss_item_sk = i_item_sk 
  	...
  	and ss_sold_date_sk = d_date_sk
	and d_date between cast(date('1998-02-05') as date) 
				and (cast(date('1998-02-05') as date) + interval '30' day)
group by 
	i_item_id
        ,i_item_desc 
        ,i_category
...
select  i_item_id
 ...
 and inv_item_sk = i_item_sk
 and d_date_sk=inv_date_sk
 and d_date between cast(date('1998-05-27') as date) and (cast(date('1998-05-27') as date) + interval '60' day)
...
select  *
 from(select w_warehouse_name
            ,i_item_id
            ,sum(case when (cast(d_date as date) < cast (date('2002-02-25') as date))
   ...
   where i_current_price between 0.99 and 1.49
     ...
     and d_date between (cast (date('2002-02-25') as date) - interval '30' day)
                    and (cast (date('2002-02-25') as date) + interval '30' day)
...
with sr_items as
 ...
 and   d_date    in 
	...
	where d_week_seq in 
		(select d_week_seq
		from date_dim
	  where d_date in (date('2001-05-26'),date('2001-09-10'),date('2001-11-05'))))
...
i_manufact_id = 26
and i_item_sk = ws_item_sk 
and d_date between date(date('1999-01-09')) and 
        (cast(date('1999-01-09') as date) + interval '90' day)
...
where 
	ws_item_sk = i_item_sk 
  	...
  	and ws_sold_date_sk = d_date_sk
	and d_date between cast(date('2001-04-16') as date) 
				and (cast(date('2001-04-16') as date) + interval '30' day)
group by 
	i_item_id
        ,i_item_desc 
        ,i_category
...
select  
   w_state
  ,i_item_id
  ,sum(case when (cast(d_date as date) < cast (date('2001-05-10') as date)) 
 ...
 and cs_warehouse_sk    = w_warehouse_sk 
 and cs_sold_date_sk    = d_date_sk
 and d_date between (cast (date('2001-05-10') as date) - interval '30' day)
                and (cast (date('2001-05-10') as date) + interval '30' day) 
 group by
    w_state,i_item_id
