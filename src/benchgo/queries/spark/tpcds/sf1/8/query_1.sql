...
with sr_items as
 ...
 and   d_date    in 
	...
	where d_week_seq in 
		(select d_week_seq
		from date_dim
	  where d_date in (date('2001-04-12'),date('2001-09-24'),date('2001-11-18'))))
...
and d_date between date(date('2002-01-29')) and 
        (cast(date('2002-01-29') as date) + interval '90' day)
...
and d_date between date(date('2000-03-27')) and 
        (cast(date('2000-03-27') as date) + interval '90' day)
...
where 
	...
	and d_date between cast(date('1998-05-07') as date) 
				and (cast(date('1998-05-07') as date) + interval '30' day)
group by 
	i_item_id
        ,i_item_desc 
        ,i_category
...
select  
   w_state
  ,i_item_id
  ,sum(case when (cast(d_date as date) < cast (date('2000-05-30') as date)) 
 ...
 and cs_sold_date_sk    = d_date_sk
 and d_date between (cast (date('2000-05-30') as date) - interval '30' day)
                and (cast (date('2000-05-30') as date) + interval '30' day) 
 group by
    w_state,i_item_id
...
where 
	ws_item_sk = i_item_sk 
  	...
  	and ws_sold_date_sk = d_date_sk
	and d_date between cast(date('2002-05-13') as date) 
				and (cast(date('2002-05-13') as date) + interval '30' day)
group by 
	i_item_id
        ,i_item_desc 
        ,i_category
...
with ssr as
 ...
 where date_sk = d_date_sk
       and d_date between cast(date('2002-08-09') as date) 
                  and (cast(date('2002-08-09') as date) + interval '14' day)
...
select  i_item_id
 ...
 and inv_item_sk = i_item_sk
 and d_date_sk=inv_date_sk
 and d_date between cast(date('2000-03-30') as date) and (cast(date('2000-03-30') as date) + interval '60' day)
...
select  *
 from(select w_warehouse_name
            ,i_item_id
            ,sum(case when (cast(d_date as date) < cast (date('1999-04-18') as date))
   ...
   where i_current_price between 0.99 and 1.49
     ...
     and d_date between (cast (date('1999-04-18') as date) - interval '30' day)
                    and (cast (date('1999-04-18') as date) + interval '30' day)
...
select  i_item_id
 ...
 where cs_item_sk = i_item_sk 
   ...
   and cs_sold_date_sk = d_date_sk
 and d_date between cast(date('2001-03-29') as date) 
 				and (cast(date('2001-03-29') as date) + interval '30' day)
 group by i_item_id
         ,i_item_desc 
         ,i_category
         ,i_class
