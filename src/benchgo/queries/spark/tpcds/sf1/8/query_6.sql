...
with ssr as
 ...
 where date_sk = d_date_sk
       and d_date between cast(date('2001-08-27') as date) 
                  and (cast(date('2001-08-27') as date) + interval '14' day)
...
select  i_item_id
 ...
 where cs_item_sk = i_item_sk 
   ...
   and cs_sold_date_sk = d_date_sk
 and d_date between cast(date('2002-04-12') as date) 
 				and (cast(date('2002-04-12') as date) + interval '30' day)
 group by i_item_id
         ,i_item_desc 
         ,i_category
         ,i_class
...
with ssr as
 ...
 where ss_sold_date_sk = d_date_sk
       and d_date between cast(date('1998-08-19') as date) 
                  and (cast(date('1998-08-19') as date) + interval '30' day)
       and ss_store_sk = s_store_sk
...
select  i_item_id
 ...
 and d_date between cast(date('1998-07-17') as date) and (cast(date('1998-07-17') as date) + interval '60' day)
...
with ss as
 ...
 from store_sales,
      ...
      store
 where ss_sold_date_sk = d_date_sk
       and d_date between cast(date('2001-08-13') as date) 
                  and (cast(date('2001-08-13') as date) + interval '30' day) 
       and ss_store_sk = s_store_sk
 group by s_store_sk)
 ,
 sr as
...
where 
	ss_item_sk = i_item_sk 
  	...
  	and ss_sold_date_sk = d_date_sk
	and d_date between cast(date('2000-02-29') as date) 
				and (cast(date('2000-02-29') as date) + interval '30' day)
group by 
	i_item_id
        ,i_item_desc 
        ,i_category
...
select  i_item_id
 ...
 and inv_item_sk = i_item_sk
 and d_date_sk=inv_date_sk
 and d_date between cast(date('1998-03-24') as date) and (cast(date('1998-03-24') as date) + interval '60' day)
...
with sr_items as
 ...
 and   d_date    in 
	...
	where d_week_seq in 
		(select d_week_seq
		from date_dim
	  where d_date in (date('2000-03-10'),date('2000-09-22'),date('2000-11-13'))))
...
i_manufact_id = 408
and i_item_sk = ws_item_sk 
and d_date between date(date('2001-02-15')) and 
        (cast(date('2001-02-15') as date) + interval '90' day)
...
where 
	ws_item_sk = i_item_sk 
  	...
  	and ws_sold_date_sk = d_date_sk
	and d_date between cast(date('2002-05-02') as date) 
				and (cast(date('2002-05-02') as date) + interval '30' day)
group by 
	i_item_id
        ,i_item_desc 
        ,i_category
