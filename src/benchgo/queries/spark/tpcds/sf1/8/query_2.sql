...
where 
	ss_item_sk = i_item_sk 
  	...
  	and ss_sold_date_sk = d_date_sk
	and d_date between cast(date('1999-05-25') as date) 
...
with ssr as
 ...
 where date_sk = d_date_sk
       and d_date between cast(date('2001-08-11') as date) 
                  and (cast(date('2001-08-11') as date) + interval '14' day)
...
with sr_items as
 ...
 and   d_date    in 
	...
	where d_week_seq in 
		(select d_week_seq
		from date_dim
	  where d_date in (date('1998-07-21'),date('1998-09-26'),date('1998-11-21'))))
...
with ssr as
 ...
 where ss_sold_date_sk = d_date_sk
       and d_date between cast(date('2000-08-12') as date) 
                  and (cast(date('2000-08-12') as date) + interval '30' day)
       and ss_store_sk = s_store_sk
       and ss_item_sk = i_item_sk
...
with ss as
 ...
 where ss_sold_date_sk = d_date_sk
       and d_date between cast(date('2002-08-17') as date) 
                  and (cast(date('2002-08-17') as date) + interval '30' day) 
       and ss_store_sk = s_store_sk
 group by s_store_sk)
 ,
 sr as
...
select  *
 from(select w_warehouse_name
            ,i_item_id
            ,sum(case when (cast(d_date as date) < cast (date('1999-05-04') as date))
   ...
   where i_current_price between 0.99 and 1.49
     ...
     and d_date between (cast (date('1999-05-04') as date) - interval '30' day)
                    and (cast (date('1999-05-04') as date) + interval '30' day)
...
select  i_item_id
 ...
 and d_date between cast(date('2002-01-20') as date) and (cast(date('2002-01-20') as date) + interval '60' day)
...
select  
   w_state
  ,i_item_id
  ,sum(case when (cast(d_date as date) < cast (date('2001-02-24') as date)) 
 ...
 and cs_warehouse_sk    = w_warehouse_sk 
 and cs_sold_date_sk    = d_date_sk
 and d_date between (cast (date('2001-02-24') as date) - interval '30' day)
...
select  i_item_id
 ...
 and inv_item_sk = i_item_sk
 and d_date_sk=inv_date_sk
 and d_date between cast(date('2002-03-22') as date) and (cast(date('2002-03-22') as date) + interval '60' day)
...
select  i_item_id
 ...
 where cs_item_sk = i_item_sk 
   ...
   and cs_sold_date_sk = d_date_sk
 and d_date between cast(date('2001-04-06') as date) 
 				and (cast(date('2001-04-06') as date) + interval '30' day)
 group by i_item_id
         ,i_item_desc 
         ,i_category
         ,i_class
...
i_manufact_id = 572
and i_item_sk = cs_item_sk 
and d_date between date(date('1998-01-17')) and 
        (cast(date('1998-01-17') as date) + interval '90' day)
...
i_manufact_id = 21
and i_item_sk = ws_item_sk 
and d_date between date(date('1998-03-24')) and 
        (cast(date('1998-03-24') as date) + interval '90' day)
