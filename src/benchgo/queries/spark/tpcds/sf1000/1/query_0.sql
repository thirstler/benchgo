...
select  i_item_id
 ...
 and d_date between cast(date('1998-04-29') as date) and (cast(date('1998-04-29') as date) + interval '60' day)
...
where 
	ss_item_sk = i_item_sk 
  	...
  	and ss_sold_date_sk = d_date_sk
	and d_date between cast(date('2002-04-01') as date) 
				and (cast(date('2002-04-01') as date) + interval '30' day)
group by 
	i_item_id
        ,i_item_desc 
        ,i_category
...
i_manufact_id = 521
and i_item_sk = ws_item_sk 
and d_date between date(date('2001-01-26')) and 
        (cast(date('2001-01-26') as date) + interval '90' day)
...
where ss_item_sk = i_item_sk and
      ...
      d_year in (1999) and
        ((i_category in ('Home','Music','Children') and
          i_class in ('paint','pop','school-uniforms')
         )
      or (i_category in ('Shoes','Books','Sports') and
          i_class in ('mens','science','guns') 
        ))
...
select  i_item_id
 ...
 where cs_item_sk = i_item_sk 
   ...
   and cs_sold_date_sk = d_date_sk
 and d_date between cast(date('1998-05-05') as date) 
 				and (cast(date('1998-05-05') as date) + interval '30' day)
 group by i_item_id
         ,i_item_desc 
         ,i_category
         ,i_class
...
select  i_item_id
 ...
 and inv_item_sk = i_item_sk
 and d_date_sk=inv_date_sk
 and d_date between cast(date('1998-01-18') as date) and (cast(date('1998-01-18') as date) + interval '60' day)
...
select c_last_name
   ...
   (select ss_ticket_number
    ...
    and store.s_county in ('Williamson County','Maverick County','Wadena County','Walker County',
                           'Kittitas County','Fairfield County','Pennington County','Huron County')
...
select c_last_name
   ...
   (select ss_ticket_number
    ...
    and date_dim.d_year in (1998,1998+1,1998+2)
    and store.s_county in ('Williamson County','Wadena County','Huron County','Richland County')
...
select  c_customer_id as customer_id
 ...
 where ca_city	        =  'Brownsville'
   and c_current_addr_sk = ca_address_sk
   and ib_lower_bound   >=  2314
   and ib_upper_bound   <=  2314 + 50000
...
with ss as (
 ...
 where i_item_id in (select
     i_item_id
from item
where i_color in ('almond','azure','chiffon'))
 and     ss_item_sk              = i_item_sk
 and     ss_sold_date_sk         = d_date_sk
 and     d_year                  = 2000
 and     d_moy                   = 4
...
select  
   w_state
  ,i_item_id
  ,sum(case when (cast(d_date as date) < cast (date('2002-05-10') as date)) 
 ...
 and cs_sold_date_sk    = d_date_sk
 and d_date between (cast (date('2002-05-10') as date) - interval '30' day)
...
select  i_item_id,
 ...
 where cs_sold_date_sk = d_date_sk and
       ...
       c_birth_month in (2,10,9,8,7,11) and
       d_year = 2002 and
       ca_state in ('NY','MI','UT'
                   ,'WV','MT','MN','AK')
...
select avg(ss_quantity)
 ...
 and((ss_addr_sk = ca_address_sk
  ...
  and ca_state in ('OH', 'FL', 'TX')
  and ss_net_profit between 100 and 200  
     ) or
     (ss_addr_sk = ca_address_sk
  and ca_country = 'United States'
  and ca_state in ('UT', 'OK', 'IL')
  and ss_net_profit between 150 and 300  
     ) or
     (ss_addr_sk = ca_address_sk
  and ca_country = 'United States'
  and ca_state in ('AL', 'NE', 'GA')
...
select  c_last_name
 ...
 from (select ss_ticket_number
       ...
       where store_sales.ss_sold_date_sk = date_dim.d_date_sk
        ...
        and date_dim.d_year in (1999,1999+1,1999+2)
        and store.s_city in ('Hillcrest','Buena Vista')
...
with sr_items as
 ...
 and   d_date    in 
	...
	where d_week_seq in 
		(select d_week_seq
		from date_dim
	  where d_date in (date('1999-05-18'),date('1999-09-26'),date('1999-11-14'))))
...
with ssr as
 ...
 where date_sk = d_date_sk
       and d_date between cast(date('2001-08-19') as date) 
                  and (cast(date('2001-08-19') as date) + interval '14' day)
