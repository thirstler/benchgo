...
select  i_item_id
 ...
 and d_date between cast(date('2000-03-27') as date) and (cast(date('2000-03-27') as date) + interval '60' day)
...
where 
	ss_item_sk = i_item_sk 
  	...
  	and ss_sold_date_sk = d_date_sk
	and d_date between cast(date('2002-02-22') as date) 
				and (cast(date('2002-02-22') as date) + interval '30' day)
group by 
	i_item_id
        ,i_item_desc 
        ,i_category
...
i_manufact_id = 950
and i_item_sk = ws_item_sk 
and d_date between date(date('2000-03-20')) and 
        (cast(date('2000-03-20') as date) + interval '90' day)
...
where 
	ws_item_sk = i_item_sk 
  	...
  	and ws_sold_date_sk = d_date_sk
	and d_date between cast(date('2000-03-01') as date) 
				and (cast(date('2000-03-01') as date) + interval '30' day)
group by 
	i_item_id
        ,i_item_desc 
        ,i_category
...
select  i_item_id
 ...
 where cs_item_sk = i_item_sk 
   ...
   and cs_sold_date_sk = d_date_sk
 and d_date between cast(date('1999-02-11') as date) 
 				and (cast(date('1999-02-11') as date) + interval '30' day)
 group by i_item_id
         ,i_item_desc 
         ,i_category
         ,i_class
...
select  i_item_id
 ...
 and inv_item_sk = i_item_sk
 and d_date_sk=inv_date_sk
 and d_date between cast(date('2002-03-23') as date) and (cast(date('2002-03-23') as date) + interval '60' day)
...
with ss as
 ...
 from store_sales,
      ...
      store
 where ss_sold_date_sk = d_date_sk
       and d_date between cast(date('1998-08-27') as date) 
                  and (cast(date('1998-08-27') as date) + interval '30' day) 
       and ss_store_sk = s_store_sk
 group by s_store_sk)
 ,
 sr as
...
select  
   w_state
  ,i_item_id
  ,sum(case when (cast(d_date as date) < cast (date('2002-02-27') as date)) 
 ...
 and cs_sold_date_sk    = d_date_sk
 and d_date between (cast (date('2002-02-27') as date) - interval '30' day)
                and (cast (date('2002-02-27') as date) + interval '30' day) 
 group by
    w_state,i_item_id
...
with sr_items as
 ...
 and   d_date    in 
	...
	where d_week_seq in 
		(select d_week_seq
		from date_dim
	  where d_date in (date('1998-02-24'),date('1998-10-12'),date('1998-11-22'))))
