 ...
 where ss_item_sk = i_item_sk
   and d_date in (select d_date
                  ...
                  where d_week_seq = (select d_week_seq 
                                      from date_dim
                                      where d_date = date(date('2002-04-19'))))
...
select  i_item_id
 ...
 and d_date between cast(date(date('1999-03-27')) as date) and (cast(date(date('1999-03-27')) as date) + interval '60' day)
...
where 
	ss_item_sk = i_item_sk 
  	...
  	and ss_sold_date_sk = d_date_sk
	and d_date between cast(date(date('2001-03-09')) as date) 
...
where 
	ws_item_sk = i_item_sk 
  	...
  	and ws_sold_date_sk = d_date_sk
	and d_date between cast(date(date('2001-02-28')) as date) 
				and (cast(date(date('2001-02-28')) as date) + interval '30' day)
group by 
	i_item_id
        ,i_item_desc 
        ,i_category
...
select  i_item_id
 ...
 and inv_item_sk = i_item_sk
 and d_date_sk=inv_date_sk
 and d_date between cast(date(date('2001-06-07')) as date) and (cast(date(date('2001-06-07')) as date) + interval '60' day)
...
select  
   w_state
  ,i_item_id
  ,sum(case when (cast(d_date as date) < cast (date(date('1998-03-13')) as date)) 
 ...
 and cs_sold_date_sk    = d_date_sk
 and d_date between (cast (date(date('1998-03-13')) as date) - interval '30' day)
                and (cast (date(date('1998-03-13')) as date) + interval '30' day) 
 group by
    w_state,i_item_id
...
with sr_items as
 ...
 and   d_date    in 
	...
	where d_week_seq in 
		(select d_week_seq
		from date_dim
	  where d_date in (date(date('1998-05-29')),date(date('1998-08-19')),date(date('1998-11-10')))))
...
with ssr as
 ...
 where date_sk = d_date_sk
       and d_date between cast(date(date('1998-08-21')) as date) 
                  and (cast(date(date('1998-08-21')) as date) + interval '14' day)
