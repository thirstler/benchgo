...
select  i_item_id
 ...
 and d_date between cast('2000-03-28' as date) 
 				and (cast('2000-03-28' as date) + 30 days)
 group by i_item_id
         ,i_item_desc 
         ,i_category
         ,i_class
...
with ssr as
 ...
 where ss_sold_date_sk = d_date_sk
       and d_date between cast('1999-08-24' as date) 
                  and (cast('1999-08-24' as date) +  30 days)
       and ss_store_sk = s_store_sk
...
and d_date between '1998-02-09' and 
        (cast('1998-02-09' as date) + 90 days)
...
select  i_item_id
 ...
 and d_date between cast('1998-04-08' as date) and (cast('1998-04-08' as date) +  60 days)
...
where 
	...
	and d_date between cast('2002-01-16' as date) 
				and (cast('2002-01-16' as date) + 30 days)
group by 
	i_item_id
        ,i_item_desc 
        ,i_category
...
select  i_item_id
 ...
 and inv_item_sk = i_item_sk
 and d_date_sk=inv_date_sk
 and d_date between cast('2000-01-23' as date) and (cast('2000-01-23' as date) +  60 days)
...
select  *
 from(select w_warehouse_name
            ,i_item_id
            ,sum(case when (cast(d_date as date) < cast ('2000-05-26' as date))
   ...
   where i_current_price between 0.99 and 1.49
     ...
     and d_date between (cast ('2000-05-26' as date) - 30 days)
...
with sr_items as
 ...
 and   d_date    in 
	...
	where d_week_seq in 
		(select d_week_seq
		from date_dim
	  where d_date in ('2000-06-07','2000-08-26','2000-11-16')))
...
i_manufact_id = 320
and i_item_sk = ws_item_sk 
and d_date between '2002-01-27' and 
        (cast('2002-01-27' as date) + 90 days)
...
select  
   w_state
  ,i_item_id
  ,sum(case when (cast(d_date as date) < cast ('2001-02-26' as date)) 
 ...
 and cs_warehouse_sk    = w_warehouse_sk 
 and cs_sold_date_sk    = d_date_sk
 and d_date between (cast ('2001-02-26' as date) - 30 days)
                and (cast ('2001-02-26' as date) + 30 days) 
 group by
    w_state,i_item_id
