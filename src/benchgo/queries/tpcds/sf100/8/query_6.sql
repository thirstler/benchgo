 ...
 where ss_item_sk = i_item_sk
   and d_date in (select d_date
                  ...
                  where d_week_seq = (select d_week_seq 
                                      from date_dim
                                      where d_date = date(date('2002-07-21'))))
...
select  i_item_id
 ...
 where cs_item_sk = i_item_sk 
   ...
   and cs_sold_date_sk = d_date_sk
 and d_date between cast(date(date('2000-03-28')) as date) 
 				and (cast(date(date('2000-03-28')) as date) + interval '30' day)
 group by i_item_id
         ,i_item_desc 
         ,i_category
         ,i_class
...
with ssr as
 ...
 where ss_sold_date_sk = d_date_sk
       and d_date between cast(date(date('1999-08-24')) as date) 
                  and (cast(date(date('1999-08-24')) as date) + interval '30' day)
       and ss_store_sk = s_store_sk
...
and d_date between date(date(date('1998-02-09'))) and 
        (cast(date(date('1998-02-09')) as date) + interval '90' day)
...
select  i_item_id
 ...
 and d_date between cast(date(date('1998-04-08')) as date) and (cast(date(date('1998-04-08')) as date) + interval '60' day)
...
with ss as
 ...
 where ss_sold_date_sk = d_date_sk
       and d_date between cast(date(date('2002-08-22')) as date) 
                  and (cast(date(date('2002-08-22')) as date) + interval '30' day) 
       and ss_store_sk = s_store_sk
 group by s_store_sk)
 ,
 sr as
...
where 
	ss_item_sk = i_item_sk 
  	...
  	and ss_sold_date_sk = d_date_sk
	and d_date between cast(date(date('2002-01-16')) as date) 
				and (cast(date(date('2002-01-16')) as date) + interval '30' day)
group by 
	i_item_id
        ,i_item_desc 
        ,i_category
...
select  i_item_id
 ...
 and inv_item_sk = i_item_sk
 and d_date_sk=inv_date_sk
 and d_date between cast(date(date('2000-01-23')) as date) and (cast(date(date('2000-01-23')) as date) + interval '60' day)
...
select  *
 from(select w_warehouse_name
            ,i_item_id
            ,sum(case when (cast(d_date as date) < cast (date(date('2000-05-26')) as date))
   ...
   where i_current_price between 0.99 and 1.49
     ...
     and d_date between (cast (date(date('2000-05-26')) as date) - interval '30' day)
...
with sr_items as
 ...
 and   d_date    in 
	...
	where d_week_seq in 
		(select d_week_seq
		from date_dim
	  where d_date in (date(date('2000-06-07')),date(date('2000-08-26')),date(date('2000-11-16')))))
...
i_manufact_id = 320
and i_item_sk = ws_item_sk 
and d_date between date(date(date('2002-01-27'))) and 
        (cast(date(date('2002-01-27')) as date) + interval '90' day)
...
where 
	ws_item_sk = i_item_sk 
  	...
  	and ws_sold_date_sk = d_date_sk
	and d_date between cast(date(date('1999-03-22')) as date) 
				and (cast(date(date('1999-03-22')) as date) + interval '30' day)
group by 
	i_item_id
        ,i_item_desc 
        ,i_category
...
select  
   w_state
  ,i_item_id
  ,sum(case when (cast(d_date as date) < cast (date(date('2001-02-26')) as date)) 
 ...
 and cs_warehouse_sk    = w_warehouse_sk 
 and cs_sold_date_sk    = d_date_sk
 and d_date between (cast (date(date('2001-02-26')) as date) - interval '30' day)
                and (cast (date(date('2001-02-26')) as date) + interval '30' day) 
 group by
    w_state,i_item_id
