...
where 
	ss_item_sk = i_item_sk 
  	...
  	and ss_sold_date_sk = d_date_sk
	and d_date between cast('1999-05-08' as date) 
				and (cast('1999-05-08' as date) + 30 days)
group by 
	i_item_id
        ,i_item_desc 
        ,i_category
...
select  *
 from(select w_warehouse_name
   ...
   where i_current_price between 0.99 and 1.49
     ...
     and d_date between (cast ('2000-05-02' as date) - 30 days)
                    and (cast ('2000-05-02' as date) + 30 days)
...
and d_date between '2000-02-09' and 
        (cast('2000-02-09' as date) + 90 days)
...
with sr_items as
 ...
 and   d_date    in 
	...
	where d_week_seq in 
		(select d_week_seq
		from date_dim
	  where d_date in ('2001-02-23','2001-09-02','2001-11-11')))
...
select  
   w_state
  ,i_item_id
  ,sum(case when (cast(d_date as date) < cast ('2002-04-28' as date)) 
 ...
 and cs_sold_date_sk    = d_date_sk
 and d_date between (cast ('2002-04-28' as date) - 30 days)
                and (cast ('2002-04-28' as date) + 30 days) 
 group by
    w_state,i_item_id
...
select  i_item_id
 ...
 and d_date between cast('1999-06-24' as date) and (cast('1999-06-24' as date) +  60 days)
