...
with sr_items as
 ...
 and   d_date    in 
	...
	where d_week_seq in 
		(select d_week_seq
		from date_dim
	  where d_date in (date(date('2001-04-25')),date(date('2001-09-22')),date(date('2001-11-16')))))
...
and d_date between date(date(date('1998-03-13'))) and 
        (cast(date(date('1998-03-13')) as date) + interval '90' day)
...
and d_date between date(date(date('1998-01-27'))) and 
        (cast(date(date('1998-01-27')) as date) + interval '90' day)
...
where 
	...
	and d_date between cast(date(date('1999-05-21')) as date) 
				and (cast(date(date('1999-05-21')) as date) + interval '30' day)
group by 
	i_item_id
        ,i_item_desc 
        ,i_category
 ...
 where ss_item_sk = i_item_sk
   and d_date in (select d_date
                  ...
                  where d_week_seq = (select d_week_seq 
                                      from date_dim
                                      where d_date = date(date('1998-02-06'))))
   and ss_sold_date_sk   = d_date_sk
 group by i_item_id),
 cs_items as
 (select i_item_id item_id
...
from
  ...
  ,call_center
where
    d_date between date(date(date('2002-5-01'))) and 
           (cast(date(date('2002-5-01')) as date) + interval '60' day)
...
with ss as
 ...
 where ss_sold_date_sk = d_date_sk
       and d_date between cast(date(date('1998-08-05')) as date) 
                  and (cast(date(date('1998-08-05')) as date) + interval '30' day) 
...
select  
   w_state
  ,i_item_id
  ,sum(case when (cast(d_date as date) < cast (date(date('1999-06-23')) as date)) 
 ...
 and cs_sold_date_sk    = d_date_sk
 and d_date between (cast (date(date('1999-06-23')) as date) - interval '30' day)
                and (cast (date(date('1999-06-23')) as date) + interval '30' day) 
 group by
    w_state,i_item_id
...
where 
	ws_item_sk = i_item_sk 
  	...
  	and ws_sold_date_sk = d_date_sk
	and d_date between cast(date(date('1998-02-08')) as date) 
				and (cast(date(date('1998-02-08')) as date) + interval '30' day)
group by 
	i_item_id
        ,i_item_desc 
        ,i_category
...
with ssr as
 ...
 where date_sk = d_date_sk
       and d_date between cast(date(date('2002-08-22')) as date) 
                  and (cast(date(date('2002-08-22')) as date) + interval '14' day)
...
select  i_item_id
 ...
 and d_date between cast(date(date('2002-03-15')) as date) and (cast(date(date('2002-03-15')) as date) + interval '60' day)
...
select  i_item_id
 ...
 and inv_item_sk = i_item_sk
 and d_date_sk=inv_date_sk
 and d_date between cast(date(date('2001-05-26')) as date) and (cast(date(date('2001-05-26')) as date) + interval '60' day)
...
with ssr as
 (select  s_store_id as store_id,
  ...
  from store_sales left outer join store_returns on
     ...
     promotion
 where ss_sold_date_sk = d_date_sk
       and d_date between cast(date(date('2001-08-06')) as date) 
                  and (cast(date(date('2001-08-06')) as date) + interval '30' day)
       and ss_store_sk = s_store_sk
       and ss_item_sk = i_item_sk
...
select  *
 from(select w_warehouse_name
            ,i_item_id
            ,sum(case when (cast(d_date as date) < cast (date(date('2002-05-06')) as date))
   ...
   where i_current_price between 0.99 and 1.49
     ...
     and d_date between (cast (date(date('2002-05-06')) as date) - interval '30' day)
                    and (cast (date(date('2002-05-06')) as date) + interval '30' day)
...
select  i_item_id
 ...
 where cs_item_sk = i_item_sk 
   ...
   and cs_sold_date_sk = d_date_sk
 and d_date between cast(date(date('2002-05-09')) as date) 
 				and (cast(date(date('2002-05-09')) as date) + interval '30' day)
 group by i_item_id
         ,i_item_desc 
         ,i_category
         ,i_class
