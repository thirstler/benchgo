...
where 
	ss_item_sk = i_item_sk 
  	...
  	and ss_sold_date_sk = d_date_sk
	and d_date between cast('2002-06-21' as date) 
				and (cast('2002-06-21' as date) + 30 days)
group by 
	i_item_id
        ,i_item_desc 
        ,i_category
...
with sr_items as
 ...
 and   d_date    in 
	...
	where d_week_seq in 
		(select d_week_seq
		from date_dim
	  where d_date in ('2002-02-08','2002-10-01','2002-11-04')))
...
with ssr as
 ...
 where ss_sold_date_sk = d_date_sk
       and d_date between cast('2001-08-14' as date) 
                  and (cast('2001-08-14' as date) +  30 days)
       and ss_store_sk = s_store_sk
       and ss_item_sk = i_item_sk
...
with ss as
 ...
 where ss_sold_date_sk = d_date_sk
       and d_date between cast('2002-08-10' as date) 
                  and (cast('2002-08-10' as date) +  30 days) 
       and ss_store_sk = s_store_sk
 group by s_store_sk)
 ,
 sr as
...
select  *
 from(select w_warehouse_name
            ,i_item_id
            ,sum(case when (cast(d_date as date) < cast ('2002-03-08' as date))
   ...
   where i_current_price between 0.99 and 1.49
     ...
     and d_date between (cast ('2002-03-08' as date) - 30 days)
...
select  i_item_id
 ...
 and d_date between cast('1999-06-21' as date) and (cast('1999-06-21' as date) +  60 days)
...
select  
 ...
 and cs_warehouse_sk    = w_warehouse_sk 
 and cs_sold_date_sk    = d_date_sk
 and d_date between (cast ('2002-06-04' as date) - 30 days)
                and (cast ('2002-06-04' as date) + 30 days) 
 group by
    w_state,i_item_id
...
where 
	ws_item_sk = i_item_sk 
  	...
  	and ws_sold_date_sk = d_date_sk
	and d_date between cast('2001-05-30' as date) 
				and (cast('2001-05-30' as date) + 30 days)
group by 
	i_item_id
        ,i_item_desc 
        ,i_category
...
select  i_item_id
 ...
 and d_date between cast('2002-02-20' as date) 
 				and (cast('2002-02-20' as date) + 30 days)
 group by i_item_id
         ,i_item_desc 
         ,i_category
         ,i_class
...
i_manufact_id = 321
and i_item_sk = cs_item_sk 
and d_date between '2001-03-21' and 
        (cast('2001-03-21' as date) + 90 days)
