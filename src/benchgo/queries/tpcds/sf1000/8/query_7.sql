...
and d_date between date(date(date('1999-03-14'))) and 
        (cast(date(date('1999-03-14')) as date) + interval '90' day)
...
and d_date between date(date(date('2002-02-13'))) and 
        (cast(date(date('2002-02-13')) as date) + interval '90' day)
...
where 
	ss_item_sk = i_item_sk 
  	...
  	and ss_sold_date_sk = d_date_sk
	and d_date between cast(date(date('1999-03-08')) as date) 
				and (cast(date(date('1999-03-08')) as date) + interval '30' day)
group by 
	i_item_id
        ,i_item_desc 
        ,i_category
...
with ssr as
 (select  s_store_id as store_id,
  ...
  from store_sales left outer join store_returns on
     ...
     promotion
 where ss_sold_date_sk = d_date_sk
       and d_date between cast(date(date('1998-08-23')) as date) 
                  and (cast(date(date('1998-08-23')) as date) + interval '30' day)
...
select  i_item_id
 ...
 where cs_item_sk = i_item_sk 
   ...
   and cs_sold_date_sk = d_date_sk
 and d_date between cast(date(date('2001-01-24')) as date) 
 				and (cast(date(date('2001-01-24')) as date) + interval '30' day)
 group by i_item_id
         ,i_item_desc 
         ,i_category
         ,i_class
...
select  i_item_id
 ...
 and d_date between cast(date(date('2002-05-01')) as date) and (cast(date(date('2002-05-01')) as date) + interval '60' day)
...
select  
   w_state
  ,i_item_id
  ,sum(case when (cast(d_date as date) < cast (date(date('2002-02-26')) as date)) 
 ...
 and cs_warehouse_sk    = w_warehouse_sk 
 and cs_sold_date_sk    = d_date_sk
 and d_date between (cast (date(date('2002-02-26')) as date) - interval '30' day)
                and (cast (date(date('2002-02-26')) as date) + interval '30' day) 
 group by
    w_state,i_item_id
 ...
 where ss_item_sk = i_item_sk
   and d_date in (select d_date
                  ...
                  where d_week_seq = (select d_week_seq 
                                      from date_dim
                                      where d_date = date(date('2000-06-08'))))
   and ss_sold_date_sk   = d_date_sk
 group by i_item_id),
 cs_items as
 (select i_item_id item_id
...
select  i_item_id
 ...
 and inv_item_sk = i_item_sk
 and d_date_sk=inv_date_sk
 and d_date between cast(date(date('2001-06-19')) as date) and (cast(date(date('2001-06-19')) as date) + interval '60' day)
...
with ssr as
 ...
 where date_sk = d_date_sk
       and d_date between cast(date(date('1998-08-05')) as date) 
                  and (cast(date(date('1998-08-05')) as date) + interval '14' day)
...
with sr_items as
 ...
 and   d_date    in 
	...
	where d_week_seq in 
		(select d_week_seq
		from date_dim
	  where d_date in (date(date('1998-06-29')),date(date('1998-09-28')),date(date('1998-11-02')))))
...
where 
	ws_item_sk = i_item_sk 
  	...
  	and ws_sold_date_sk = d_date_sk
	and d_date between cast(date(date('2002-05-01')) as date) 
				and (cast(date(date('2002-05-01')) as date) + interval '30' day)
group by 
	i_item_id
        ,i_item_desc 
        ,i_category
...
with ss as
 ...
 from store_sales,
      ...
      store
 where ss_sold_date_sk = d_date_sk
       and d_date between cast(date(date('1998-08-16')) as date) 
                  and (cast(date(date('1998-08-16')) as date) + interval '30' day) 
       and ss_store_sk = s_store_sk
 group by s_store_sk)
 ,
 sr as
