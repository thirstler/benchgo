 ...
 where ss_item_sk = i_item_sk
   and d_date in (select d_date
                  ...
                  where d_week_seq = (select d_week_seq 
                                      from date_dim
                                      where d_date = date(date('2002-04-23'))))
...
select  i_item_id
 ...
 and d_date between cast(date(date('1998-04-29')) as date) and (cast(date(date('1998-04-29')) as date) + interval '60' day)
...
where 
	ss_item_sk = i_item_sk 
  	...
  	and ss_sold_date_sk = d_date_sk
	and d_date between cast(date(date('2002-04-01')) as date) 
				and (cast(date(date('2002-04-01')) as date) + interval '30' day)
group by 
	i_item_id
        ,i_item_desc 
        ,i_category
...
i_manufact_id = 521
and i_item_sk = ws_item_sk 
and d_date between date(date(date('2001-01-26'))) and 
        (cast(date(date('2001-01-26')) as date) + interval '90' day)
...
select  i_item_id
 ...
 where cs_item_sk = i_item_sk 
   ...
   and cs_sold_date_sk = d_date_sk
 and d_date between cast(date(date('1998-05-05')) as date) 
 				and (cast(date(date('1998-05-05')) as date) + interval '30' day)
 group by i_item_id
         ,i_item_desc 
         ,i_category
         ,i_class
...
select  i_item_id
 ...
 and inv_item_sk = i_item_sk
 and d_date_sk=inv_date_sk
 and d_date between cast(date(date('1998-01-18')) as date) and (cast(date(date('1998-01-18')) as date) + interval '60' day)
...
select  
   w_state
  ,i_item_id
  ,sum(case when (cast(d_date as date) < cast (date(date('2002-05-10')) as date)) 
 ...
 and cs_sold_date_sk    = d_date_sk
 and d_date between (cast (date(date('2002-05-10')) as date) - interval '30' day)
...
with sr_items as
 ...
 and   d_date    in 
	...
	where d_week_seq in 
		(select d_week_seq
		from date_dim
	  where d_date in (date(date('1999-05-18')),date(date('1999-09-26')),date(date('1999-11-14')))))
...
with ssr as
 ...
 where date_sk = d_date_sk
       and d_date between cast(date(date('2001-08-19')) as date) 
                  and (cast(date(date('2001-08-19')) as date) + interval '14' day)
