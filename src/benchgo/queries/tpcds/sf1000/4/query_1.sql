...
with sr_items as
 ...
 and   d_date    in 
	...
	where d_week_seq in 
		(select d_week_seq
		from date_dim
	  where d_date in (date(date('2002-02-01')),date(date('2002-10-21')),date(date('2002-11-04')))))
...
and d_date between date(date(date('1999-02-15'))) and 
        (cast(date(date('1999-02-15')) as date) + interval '90' day)
...
and d_date between date(date(date('2002-03-04'))) and 
        (cast(date(date('2002-03-04')) as date) + interval '90' day)
...
where 
	...
	and d_date between cast(date(date('2002-06-13')) as date) 
				and (cast(date(date('2002-06-13')) as date) + interval '30' day)
group by 
	i_item_id
        ,i_item_desc 
        ,i_category
 ...
 where ss_item_sk = i_item_sk
   and d_date in (select d_date
                  ...
                  where d_week_seq = (select d_week_seq 
                                      from date_dim
                                      where d_date = date(date('2001-06-14'))))
   and ss_sold_date_sk   = d_date_sk
 group by i_item_id),
 cs_items as
 (select i_item_id item_id
...
with ss as
 ...
 where ss_sold_date_sk = d_date_sk
       and d_date between cast(date(date('2001-08-07')) as date) 
                  and (cast(date(date('2001-08-07')) as date) + interval '30' day) 
...
select  
   w_state
  ,i_item_id
  ,sum(case when (cast(d_date as date) < cast (date(date('2002-05-16')) as date)) 
 ...
 and cs_sold_date_sk    = d_date_sk
 and d_date between (cast (date(date('2002-05-16')) as date) - interval '30' day)
                and (cast (date(date('2002-05-16')) as date) + interval '30' day) 
 group by
    w_state,i_item_id
...
where 
	ws_item_sk = i_item_sk 
  	...
  	and ws_sold_date_sk = d_date_sk
	and d_date between cast(date(date('2000-05-14')) as date) 
				and (cast(date(date('2000-05-14')) as date) + interval '30' day)
group by 
	i_item_id
        ,i_item_desc 
        ,i_category
...
with ssr as
 ...
 where date_sk = d_date_sk
       and d_date between cast(date(date('2002-08-05')) as date) 
                  and (cast(date(date('2002-08-05')) as date) + interval '14' day)
...
select  i_item_id
 ...
 and d_date between cast(date(date('2001-02-13')) as date) and (cast(date(date('2001-02-13')) as date) + interval '60' day)
...
select  i_item_id
 ...
 and inv_item_sk = i_item_sk
 and d_date_sk=inv_date_sk
 and d_date between cast(date(date('2001-04-14')) as date) and (cast(date(date('2001-04-14')) as date) + interval '60' day)
...
select  *
 from(select w_warehouse_name
            ,i_item_id
            ,sum(case when (cast(d_date as date) < cast (date(date('1998-05-08')) as date))
   ...
   where i_current_price between 0.99 and 1.49
     ...
     and d_date between (cast (date(date('1998-05-08')) as date) - interval '30' day)
                    and (cast (date(date('1998-05-08')) as date) + interval '30' day)
