...
with ssr as
 ...
 where date_sk = d_date_sk
       and d_date between cast(date(date('2002-08-12')) as date) 
                  and (cast(date(date('2002-08-12')) as date) + interval '14' day)
 ...
 where ss_item_sk = i_item_sk
   and d_date in (select d_date
                  ...
                  where d_week_seq = (select d_week_seq 
                                      from date_dim
                                      where d_date = date(date('1999-05-11'))))
   and ss_sold_date_sk   = d_date_sk
 group by i_item_id),
 cs_items as
 (select i_item_id item_id
...
select  i_item_id
 ...
 and d_date between cast(date(date('1999-05-26')) as date) and (cast(date(date('1999-05-26')) as date) + interval '60' day)
...
select  *
 from(select w_warehouse_name
            ,i_item_id
            ,sum(case when (cast(d_date as date) < cast (date(date('2000-04-16')) as date))
   ...
   where i_current_price between 0.99 and 1.49
     ...
     and inv_warehouse_sk   = w_warehouse_sk
     and inv_date_sk    = d_date_sk
     and d_date between (cast (date(date('2000-04-16')) as date) - interval '30' day)
                    and (cast (date(date('2000-04-16')) as date) + interval '30' day)
...
select  i_item_id
 ...
 and inv_item_sk = i_item_sk
 and d_date_sk=inv_date_sk
 and d_date between cast(date(date('2001-05-12')) as date) and (cast(date(date('2001-05-12')) as date) + interval '60' day)
...
select  
   w_state
  ,i_item_id
  ,sum(case when (cast(d_date as date) < cast (date(date('1999-06-03')) as date)) 
 ...
 and cs_sold_date_sk    = d_date_sk
 and d_date between (cast (date(date('1999-06-03')) as date) - interval '30' day)
                and (cast (date(date('1999-06-03')) as date) + interval '30' day) 
 group by
    w_state,i_item_id
...
with sr_items as
 ...
 and   d_date    in 
	...
	where d_week_seq in 
		(select d_week_seq
		from date_dim
	  where d_date in (date(date('1999-06-05')),date(date('1999-10-03')),date(date('1999-11-18')))))
...
where 
	ws_item_sk = i_item_sk 
  	...
  	and ws_sold_date_sk = d_date_sk
	and d_date between cast(date(date('2002-06-04')) as date) 
...
i_manufact_id = 212
and i_item_sk = cs_item_sk 
and d_date between date(date(date('2000-03-03'))) and 
        (cast(date(date('2000-03-03')) as date) + interval '90' day)
...
select  i_item_id
 ...
 where cs_item_sk = i_item_sk 
   ...
   and cs_sold_date_sk = d_date_sk
 and d_date between cast(date(date('1998-04-05')) as date) 
 				and (cast(date(date('1998-04-05')) as date) + interval '30' day)
 group by i_item_id
         ,i_item_desc 
         ,i_category
         ,i_class
