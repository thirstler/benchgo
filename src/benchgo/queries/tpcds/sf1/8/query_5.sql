...
select  *
 from(select w_warehouse_name
            ,i_item_id
            ,sum(case when (cast(d_date as date) < cast (date(date('1998-06-05')) as date))
   ...
   where i_current_price between 0.99 and 1.49
     ...
     and d_date between (cast (date(date('1998-06-05')) as date) - interval '30' day)
                    and (cast (date(date('1998-06-05')) as date) + interval '30' day)
 ...
 where ss_item_sk = i_item_sk
   and d_date in (select d_date
                  ...
                  where d_week_seq = (select d_week_seq 
                                      from date_dim
                                      where d_date = date(date('2002-02-10'))))
   and ss_sold_date_sk   = d_date_sk
 group by i_item_id),
 cs_items as
 (select i_item_id item_id
...
with ssr as
 ...
 where ss_sold_date_sk = d_date_sk
       and d_date between cast(date(date('2000-08-29')) as date) 
                  and (cast(date(date('2000-08-29')) as date) + interval '30' day)
       and ss_store_sk = s_store_sk
       and ss_item_sk = i_item_sk
...
and d_date between date(date(date('2000-01-11'))) and 
        (cast(date(date('2000-01-11')) as date) + interval '90' day)
...
and d_date between date(date(date('2002-03-25'))) and 
        (cast(date(date('2002-03-25')) as date) + interval '90' day)
...
with sr_items as
 ...
 and   d_date    in 
	...
	where d_week_seq in 
		(select d_week_seq
		from date_dim
	  where d_date in (date(date('2001-04-01')),date(date('2001-10-13')),date(date('2001-11-15')))))
...
select  
   w_state
  ,i_item_id
  ,sum(case when (cast(d_date as date) < cast (date(date('2001-06-24')) as date)) 
 ...
 and cs_sold_date_sk    = d_date_sk
 and d_date between (cast (date(date('2001-06-24')) as date) - interval '30' day)
                and (cast (date(date('2001-06-24')) as date) + interval '30' day) 
 group by
    w_state,i_item_id
...
select  i_item_id
 ...
 where cs_item_sk = i_item_sk 
   ...
   and cs_sold_date_sk = d_date_sk
 and d_date between cast(date(date('2001-05-16')) as date) 
 				and (cast(date(date('2001-05-16')) as date) + interval '30' day)
 group by i_item_id
         ,i_item_desc 
         ,i_category
         ,i_class
...
where 
	ws_item_sk = i_item_sk 
  	...
  	and ws_sold_date_sk = d_date_sk
	and d_date between cast(date(date('2001-04-27')) as date) 
				and (cast(date(date('2001-04-27')) as date) + interval '30' day)
group by 
	i_item_id
        ,i_item_desc 
        ,i_category
...
select  i_item_id
 ...
 and d_date between cast(date(date('2002-01-18')) as date) and (cast(date(date('2002-01-18')) as date) + interval '60' day)
...
select  i_item_id
 ...
 and inv_item_sk = i_item_sk
 and d_date_sk=inv_date_sk
 and d_date between cast(date(date('2000-06-24')) as date) and (cast(date(date('2000-06-24')) as date) + interval '60' day)
